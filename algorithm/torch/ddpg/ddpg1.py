...
class DDPG(Agent):
    ...
    def normalizer_update(self, batch):
        """Update observation normalizer using both current and next state observations"""
        if self._state_preprocessor:
            # Concatenate current and next observations along batch dimension
            combined_obs = torch.cat([batch["states"], batch["next_states"]], dim=0)
            self._state_preprocessor.update(combined_obs)
    ...
    def train(self, batch):
        ...
        # Compute target values
        with torch.no_grad():
            next_actions, _, _ = self.target_policy.act({"states": next_states}, role="target_policy")
            target_q_values, _, _ = self.target_critic.act(
                {"states": next_states, "taken_actions": next_actions}, role="target_critic"
            )
            target_values = (
                    rewards
                    + self._discount_factor * (terminated | truncated).logical_not() * target_q_values
            )

        # Compute critic loss
        critic_values, _, _ = self.critic.act(
            {"states": states, "taken_actions": actions}, role="critic"
        )
        critic_loss = F.mse_loss(critic_values, target_values)
        ...
        # Compute policy loss
        new_actions, _, _ = self.policy.act({"states": states}, role="policy")
        policy_q_values, _, _ = self.critic.act(
            {"states": states, "taken_actions": new_actions}, role="critic"
        )
